...
starting_date: date = date(2016, 1, 1)
ending_date: date = date(2017, 12, 31)
        ...
        case "sentiment":
            for sector in sectors:
                for rv in reference_variables:
                    observation_dataset_columns.extend(
                        [
                            f"{sector}_{column}" for column in reference_variables
                        ]
                    )
...
def get_actor(
    ...
    lstm_3 = layers.LSTM(64)(lstm_2)
    outputs = layers.Dense(32)(lstm_3)
    outputs = layers.Dense(n_actions, activation="softmax")(outputs)
...
def get_critic(
    ...
    lstm_3 = layers.LSTM(64)(lstm_2)
    outputs = layers.Dense(32)(lstm_3)
    outputs = layers.Dense(n_actions, activation="linear")(outputs)
